package cn.edu.gdmec.s07150728.myweather;
...
/**
 * Created by Administartor on 2016/12/18.
 */
public class GetWeatherInfoTask extends AsyncTask<String,Void,List<Map<String,Object>>> {
    ...
    private static String key = "&key=44db1f46fda8508240412474cebc85ba";
    public GetWeatherInfoTask(Activity context){
        this.context = context;
        progressDialog = new ProgressDialog(context);
        progressDialog.setMessage("正在获取天气，请销后.....");
            ...
            if(response.getStatusLine().getStatusCode() == 200){
                ...
                if(jsondata.getInt("resultcode") == 200){
                   JSONObject result = jsondata.getJSONObject("result");
                    JSONArray weatherList = new JSONArray("future");
                    for(int i=0;i<7;i++){
